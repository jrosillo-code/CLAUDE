...
export function BriefHereButton({ bubbleLeft = false, compact = false }: { bubbleLeft?: boolean; compact?: boolean }) {
  ...
  return (
    <button
      ...
      className={`group relative grid shrink-0 place-items-center rounded-full transition-colors ${
        compact ? "h-8 w-8" : "h-11 w-11 shadow-float backdrop-blur"
      } ${
        searched ? "wp-brief-glow bg-accent text-paper" : compact ? "bg-paper-2 text-accent hover:bg-line" : "bg-paper/90 text-accent hover:bg-paper"
      } ${busy ? "opacity-70" : ""}`}
      data-testid="button-brief-here"
      data-glow={searched ? "1" : undefined}
    >
      <BriefIcon size={compact ? 16 : 20} className={busy ? "animate-pulse" : ""} />
      <span
